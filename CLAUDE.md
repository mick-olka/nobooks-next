...
Knowledge-base content that's actually editable in-app is rows in the Supabase **`wiki_pages`** table, discriminated by `type` (`WikiPageType`: `region` / `wiki` / `feature` / `rule` / `history` / `faq` — see `app/types/wiki-pages.ts`). `/regions`, `/history`, `/features`, and `/wiki/[id]` read `wiki_pages` — the first three via `getCachedWikiPages(type)`, `/wiki/[id]` via `getCachedWikiPageByUrlName()` (both in `app/lib/data/wiki-cache.ts`); `/wiki/[id]` looks pages up by **`url_name`** (a slug), not the numeric id.

**`/faq` and `/rules` do NOT read `wiki_pages`** — they read local Markdown/data files: `/faq` reads `app/faq/data/*.md` directly off disk (`readdirSync`/`readFileSync` in `app/faq/page.tsx`) and parses them client-side with `useFeaturesList` (`app/utils/hooks/use-features-list.ts`); `/rules` reads the static array in `app/rules/data.ts`. Don't assume these two routes hit the database — but note `/features` (despite the similarly-named `app/features/data/*.md` files, which are unused dead weight) does read `wiki_pages` like `/regions` and `/history`.
