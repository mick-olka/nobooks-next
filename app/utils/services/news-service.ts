...
function formatSupabaseError(error: unknown): string {
  ...
  if (typeof error === "object" && error !== null) {
    ...
    return [message, details, hint, code].filter(Boolean).join(" | ") || JSON.stringify(error);
  }
  return String(error);
}
...
export async function syncTelegramNews(): Promise<TelegramNewsSyncResult> {
  ...
  if (latestNewsError) {
    console.error("Error loading latest Telegram news:", formatSupabaseError(latestNewsError));
  }
  ...
  if (insertError) {
    console.error("Error inserting Telegram news:", formatSupabaseError(insertError));
